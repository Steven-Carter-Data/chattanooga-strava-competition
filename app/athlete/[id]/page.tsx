    ...
    <div className="min-h-screen">
      ...
      <div className="bourbon-hero text-foreground relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 py-10 relative z-10">
          ...
          <Link
            href="/"
            className="inline-flex items-center text-muted hover:text-gold mb-8 transition-colors duration-300 font-body uppercase tracking-wider text-sm"
          >
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left flex-1">
              {/* Athlete Name */}
              <h1 className="text-4xl md:text-6xl font-display mb-3 tracking-widest uppercase text-foreground">
              ...
              <p className="text-muted font-body uppercase tracking-wider">Athlete Performance Dashboard</p>
            </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-10">
          <div className="card p-8 text-center">
            <div className="text-xs text-muted font-body uppercase tracking-widest mb-3">Total Points</div>
            <div className="text-6xl font-display gradient-text">{data.summary.total_points.toFixed(1)}</div>
          </div>
          <div className="card p-8 text-center">
            <div className="text-xs text-muted font-body uppercase tracking-widest mb-3">Total Activities</div>
            <div className="text-6xl font-display text-foreground">{data.summary.activity_count}</div>
          </div>
        </div>
        ...
        <div className="mb-10">
          ...
          <p className="text-sm text-muted font-body mt-3 tracking-wide">
        ...
        {data.athlete.hr_zones && (
          <div className="card p-8 mb-10">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-xl font-display text-gold tracking-wider uppercase">
                  Strava Heart Rate Zones Configuration
                </h3>
                <div className="h-px w-48 bg-gold/30 mt-2"></div>
              </div>
            </div>

            <div className="mb-6">
              <span className="inline-block px-4 py-2 bg-background border border-gold/30 text-gold text-xs font-body font-semibold uppercase tracking-widest">
            ...
            <div className="grid grid-cols-5 gap-3">
              {data.athlete.hr_zones.zones.map((zone, index) => {
                ...
                return (
                  <div key={index} className={`bg-background border-2 ${zoneColors[index].border} p-4 text-center`}>
                    <div className={`text-xs font-display mb-2 ${zoneColors[index].text} uppercase tracking-wider`}>
                      Zone {toRoman(index + 1)}
                    </div>
                    <div className="text-lg font-body font-bold text-foreground">
                      {displayMin} - {displayMax}
                    </div>
                    <div className="text-xs text-muted font-body uppercase tracking-wider mt-1">bpm</div>
                  </div>
                );
              })}
            </div>
        ...
        <div className="card p-8 mb-10">
          <div className="flex items-center gap-4 mb-8">
            <h2 className="text-2xl md:text-3xl font-display gradient-text tracking-wider uppercase">
              Heart Rate Zone Distribution
            </h2>
          </div>
          <div className="h-px w-48 bg-gold/30 mb-8"></div>
              ...
              return (
                <div key={zone}>
                  <div className="flex justify-between items-center mb-3">
                    <span className={`font-display text-lg ${zoneColors[zone - 1].text} uppercase tracking-wider`}>
                      Zone {toRoman(zone)}
                      {hrZone && (
                        <span className="ml-3 text-xs font-body text-muted normal-case tracking-normal">
                  ...
                  <div className="w-full bg-background border border-gold/20 h-4">
                    <div
        ...
        {Object.keys(data.sport_breakdown).length > 0 && (
          <div className="card p-8 mb-10">
            <h2 className="text-2xl md:text-3xl font-display mb-2 gradient-text tracking-wider uppercase">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {Object.entries(data.sport_breakdown).map(([sport, stats]) => (
                <div key={sport} className="bg-background border border-gold/20 p-6 hover:border-gold/40 transition-all duration-300">
                  <h3 className="font-display text-xl mb-4 text-gold tracking-wider uppercase">{formatSportType(sport)}</h3>
                  <div className="space-y-3 text-sm font-body">
                    <div className="flex justify-between border-b border-gold/10 pb-2">
                      <span className="text-muted uppercase tracking-wider text-xs">Activities</span>
                      <span className="font-semibold text-foreground">{stats.count}</span>
                    </div>
                    <div className="flex justify-between border-b border-gold/10 pb-2">
                      <span className="text-muted uppercase tracking-wider text-xs">Points</span>
                      <span className="font-semibold gradient-text">{stats.points.toFixed(1)}</span>
                    </div>
                    <div className="flex justify-between border-b border-gold/10 pb-2">
                      <span className="text-muted uppercase tracking-wider text-xs">Distance</span>
            ...
            <div className="space-y-4">
              {data.recent_activities.map((activity) => {
                ...
                return (
                  <div
                    key={activity.id}
                    className="bg-background border border-gold/20 p-6 hover:border-gold/40 transition-all duration-300"
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="font-display text-xl text-foreground tracking-wide">{activity.name}</h3>
                      ...
                      <div className="text-right">
                        <div className="text-3xl font-display gradient-text">{activity.zone_points?.toFixed(1) || '0.0'}</div>
                        <div className="text-xs text-muted font-body uppercase tracking-wider">points</div>
                      </div>
                    </div>

                    <div className="grid grid-cols-3 gap-4 text-sm mb-4 pb-4 border-b border-gold/10">
                      <div>
                        <div className="text-muted font-body uppercase tracking-wider text-xs mb-1">Distance</div>
                        <div className="font-body font-semibold text-foreground">{((activity.distance_m || 0) / 1609.34).toFixed(2)} mi</div>
                      </div>
                      <div>
                        <div className="text-muted font-body uppercase tracking-wider text-xs mb-1">Time</div>
                        <div className="font-body font-semibold text-foreground">
                          {Math.floor((activity.moving_time_s || 0) / 3600)}h {Math.floor(((activity.moving_time_s || 0) % 3600) / 60)}m
                        </div>
                      </div>
                      <div>
                        <div className="text-muted font-body uppercase tracking-wider text-xs mb-1">Avg HR</div>
                        <div className="font-body font-semibold text-foreground">{activity.average_heartrate ? `${Math.round(activity.average_heartrate)} bpm` : 'N/A'}</div>
                      </div>
                    </div>
                    ...
                    {activityTotalZoneTime > 0 && (
                      <div>
                        <div className="text-xs font-display text-gold mb-3 uppercase tracking-wider">HR Zone Distribution</div>
                          ...
                          {[1, 2, 3, 4, 5].map((zone) => {
                            ...
                            return (
                              <div key={zone} className="text-center">
                                <div className={`text-xs font-display mb-2 ${zoneColors[zone - 1].text} uppercase`}>
                                  Z{toRoman(zone)}
                                </div>
                                <div className={`w-full bg-background border border-gold/10 h-2 mb-2`}>
                                  <div
                                    className={`h-full transition-all duration-500 ${zoneColors[zone - 1].bg}`}
                                    style={{ width: `${percentage}%` }}
                                  />
